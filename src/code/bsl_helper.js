...
class bslHelper {
	 ...
	 */
	 getLoopsVarNames() {
		...
		let names = [];
		let matches = Finder.findMatches(this.model, '(?:для каждого|for each)\\s+([a-zA-Z0-9\u0410-\u044F_]+)\\s+(?:из|in)');
		matches = matches.concat(Finder.findMatches(this.model, '(?:для|for)\\s+([a-zA-Z0-9\u0410-\u044F_]+)\\s+=.*(?:по|to)'));
