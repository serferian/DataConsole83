...
Процедура ВыполнитьСерверныйКодВФоне(Команда, АлгоритмВыполнения = Неопределено)
	ВыполнитьВФонеНаСервере(АлгоритмВыполнения);
КонецПроцедуры
...
Процедура ВыполнитьКод(ВыполнитьКодСоСтраницы, ДополнительныеПараметры = Неопределено,
	...
	Если СтраницаКода = "КодКлиента1С" Тогда
		...
		Текст = ?(АлгоритмВыполнения = Неопределено,
			View().getText(), Алгоритм[СтраницаКода]);
	...
	Если СтраницаКода = "КодСервера1С" Тогда
		...
		Текст = ?(АлгоритмВыполнения = Неопределено, ПолучитьТекст(), Алгоритм[СтраницаКода]);
...
Процедура ВыполнитьКодЗапроса(Знач Алгоритм, Знач ДополнительныеПараметры,
	ИспользоватьТекстРедактора = Истина)
...
Процедура ВыполнитьВФонеНаСервере(АлгоритмВыполнения = Неопределено)
	...
	Алгоритм = СтрокаАлгоритма.КодФоновойОбработки;
...
Процедура ПозиционироватьВТекстеИмяТаблицы(ИмяТаблицы)
	...
	Сообщить("[DataConsole83] Позиционирование: таблица=" + Строка(ИмяТаблицы) + ", страница=" + Строка(СтраницаКода));
	Алгоритм = Элементы.Алгоритмы.ТекущиеДанные;
	Если Алгоритм = Неопределено Тогда
		Сообщить("[DataConsole83] Позиционирование пропущено: текущий алгоритм не определен");
		Возврат;
	КонецЕсли;
	Сообщить("[DataConsole83] Текущий алгоритм определен, код запроса заполнен=" + Строка(ЗначениеЗаполнено(Алгоритм.КодЗапроса)));
	Если СтраницаКода <> "КодЗапроса" Тогда
		Сообщить("[DataConsole83] Позиционирование пропущено: активна страница " + Строка(СтраницаКода));
		Возврат;
	КонецЕсли;
	...
	Если СтрНачинаетсяС(ИмяТаблицы, "РЕЗУЛЬТАТ") Тогда
		...
		Исключение
			Сообщить("[DataConsole83] Позиционирование результата пропущено: некорректное имя " + Строка(ИмяТаблицы));
			Возврат;
		КонецПопытки;
		Сообщить("[DataConsole83] Результат: номер=" + Строка(НомерРезультата));
		КоличествоРазделителей = НомерРезультата - 1;
		Если КоличествоРазделителей < 1 Тогда
			КоличествоРазделителей = 1;
		КонецЕсли;
		ТекстЗапроса = View().getText();
		Сообщить("[DataConsole83] Monaco: длина текста=" + Строка(СтрДлина(ТекстЗапроса)) + ", строк=" + Строка(View().getLineCount()));
		ПозицияПоиска = 1;
		Для НомерРазделителя = 1 По КоличествоРазделителей Цикл
			ПозицияРазделителя = СтрНайти(ТекстЗапроса, ";", , ПозицияПоиска);
			Если ПозицияРазделителя = 0 Тогда
				Сообщить("[DataConsole83] Позиционирование результата пропущено: разделитель №" + Строка(НомерРазделителя) + " не найден");
				Возврат;
			КонецЕсли;
		...
		Сообщить("[DataConsole83] Позиция результата: разделитель=" + Строка(ПозицияРазделителя) + ", строка=" + Строка(НомерСтрокиПозиции) + ", колонка=" + Строка(КолонкаПозиции));
	...
	Иначе
		...
		Сообщить("[DataConsole83] Поиск таблицы: шаблон=ПОМЕСТИТЬ " + Строка(ИмяТаблицы) + ", позиция=" + Строка(НачальнаяПозиция));
	КонецЕсли;
	Если НачальнаяПозиция < 1 Тогда
		Сообщить("[DataConsole83] Позиционирование пропущено: таблица не найдена в тексте");
		Возврат;
	КонецЕсли;
...
Процедура УстановитьЗаголовокФормы()
	...
	Заголовок = СтрШаблон("DataConsole83 v%1 — %2 %3",
		ВерсияОбработки,
...
Процедура ОбработатьКомандуТаблицРабочейОбласти(ПараметрыСобытия)
	...
	Исключение
		Сообщить("[DataConsole83] Команда таблицы пропущена: не удалось разобрать параметры");
		Возврат;
	КонецПопытки;
	Сообщить("[DataConsole83] Команда таблицы: действие=" + Действие + ", tableId=" + ИдентификаторТаблицы);
	...
	Если СтрокаТаблиц = Неопределено Тогда
		Сообщить("[DataConsole83] Таблица не найдена: tableId=" + ИдентификаторТаблицы);
		Возврат;
	КонецЕсли;
	Сообщить("[DataConsole83] Таблица найдена: имя=" + Строка(СтрокаТаблиц.ИмяТаблицы));
	...
	Попытка
		АктивныйДокумент = QueryView().DataConsoleApp.getActiveDocument();
		Если АктивныйДокумент <> Неопределено Тогда
			Сообщить("[DataConsole83] Активный HTML-документ: algorithmId=" + Строка(АктивныйДокумент.algorithmId));
		...
		КонецЕсли;
	Исключение
		Сообщить("[DataConsole83] Не удалось получить активный HTML-документ, использую текущий алгоритм формы");
		Алгоритм = Элементы.Алгоритмы.ТекущиеДанные;
	КонецПопытки;
	Если Алгоритм = Неопределено Тогда
		Сообщить("[DataConsole83] Команда таблицы пропущена: алгоритм не определен");
		Возврат;
	КонецЕсли;
	Сообщить("[DataConsole83] Активный алгоритм для таблицы получен");
...
Процедура ОбработатьКомандуРабочейОбласти(ПараметрыСобытия)
	...
	Если ВидДокумента = "query" Тогда
		Если Действие = "execute-query" Тогда
			ВыполнитьКод("КодЗапроса", Неопределено, Алгоритм);
		ИначеЕсли Действие = "execute-query-to-cursor" Тогда
			ВыполнитьКод("КодЗапроса", "ДоКурсора", Алгоритм);
		ИначеЕсли Действие = "execute-query-package" Тогда
			ВыполнитьКод("КодЗапроса", "ПакетныйЗапрос", Алгоритм);
	...
	ИначеЕсли ВидДокумента = "client" Тогда
		Если Действие = "execute-client" Тогда
			ВыполнитьКод("КодКлиента1С", Неопределено, Алгоритм);
	...
	ИначеЕсли ВидДокумента = "server" Тогда
		Если Действие = "execute-server" Тогда
			ВыполнитьКод("КодСервера1С", Неопределено, Алгоритм);
		ИначеЕсли Действие = "insert-iteration-example" Тогда
			ПримерКода(Неопределено, Алгоритм, ИмяПоля);
			ОбновитьТекстДокументаHTML(АлгоритмУИД, ВидДокумента);
			ЭтотОбъект.Модифицированность = Истина;
	...
	ИначеЕсли ВидДокумента = "background" Тогда
		Если Действие = "execute-background" Тогда
			ВыполнитьСерверныйКодВФоне(Неопределено, Алгоритм);
