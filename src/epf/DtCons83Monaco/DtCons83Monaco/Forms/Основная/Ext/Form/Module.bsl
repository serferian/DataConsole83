...
&НаКлиенте
Функция ПодсказкаКодаЗапросаПередВыполнением()
...
&НаКлиенте
Функция ПодсказкаКодаКлиентСервер()
...
&НаКлиенте
Функция ПодсказкаКодаФоноваяОбработка()
...
Процедура АлгоритмыПриАктивизацииЯчейки(Элемент)
	...
	ИначеЕсли Поле = Элементы.АлгоритмыЕстьКодЗапроса Или Поле = Элементы.АлгоритмыКодЗапроса Тогда
...
Процедура ВыгрузкаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если СодержимоеЯчейки = "ТаблицаЗначений" И ЗначениеЗаполнено(Адрес) Тогда
		ФормаВложеннойТаблицы = ПолучитьФорму("ВнешняяОбработка.DtCons83Monaco.Форма.ФормаВложеннойТаблицы", Неопределено, ЭтаФорма);
...
Функция ВыполнитьЗапросСЗамерами(КодЗапроса, ПараметрыЗапроса)
	...
	Возврат ?(ЗначениеЗаполнено(ПакетЗапроса.ТаблицаДляПомещения), Неопределено, Данные);
КонецФункции
...
Процедура ПолучитьКодПродолжить(пНазваниеЗапроса, ДополнительныеПараметры) Экспорт
	...
	ФормаКода = ПолучитьФорму("ВнешняяОбработка.DtCons83Monaco.Форма.ФормаКодаЗапроса",, ЭтаФорма);
...
Процедура ВывестиВТабДок(Команда)
	...
	ОписаниеОЗавершении = Новый ОписаниеОповещения("ПолучениФайлаЗавершение", ЭтотОбъект, Начало);
	
	НачатьПолучениеФайлаССервера(ОписаниеОЗавершении, 
		Адрес, 
		ИмяФайла);
КонецПроцедуры

&НаКлиенте
Процедура ПолучениФайлаЗавершение(ПолученныйФайл, ДополнительныеПараметры) Экспорт
	ИмяФайла = ПолученныйФайл.ПолноеИмя;
	
	Сообщить("Записан за " + (ТекущаяДата() - ДополнительныеПараметры) + " сек");
...
Процедура СохранитьВCSV(Команда)
	...
	Сообщить("Записан за " + (ТекущаяДата() - Начало) + " сек");
