...
Процедура ВыполнитьКод(ВыполнитьКодСоСтраницы, ДополнительныеПараметры = Неопределено,
	...
	Если СтраницаКода = "КодЗапроса" Тогда
		ВыполнитьКодЗапроса(Алгоритм, ДополнительныеПараметры, ИспользоватьТекстРедактора, ТекстДокумента);
		Возврат;
	КонецЕсли;
...
&НаСервере
Функция ВыполнитьЗапросСЗамерами(КодЗапроса, ПараметрыЗапроса)
	
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	АлгоритмПередВыполнением = Алгоритмы
		.НайтиПоИдентификатору(Элементы.Алгоритмы.ТекущаяСтрока)
...
Процедура ВыгрузитьДанныеНаСервере(КодЗапроса, ПараметрыЗапроса = Неопределено)
	Таблицы.Очистить();
	РезультатПакет = ВыполнитьЗапросСЗамерами(КодЗапроса, ПараметрыЗапроса);
...
Процедура ОбработатьКомандуРабочейОбласти(ПараметрыСобытия)
	...
	Попытка
		...
		ВидДокумента = Строка(ПараметрыСобытия.documentKind);
		Действие = Строка(ПараметрыСобытия.action);
		Если ПараметрыСобытия.Свойство("documentText", ЗначениеТекстаДокумента) Тогда
			ТекстДокумента = Строка(ЗначениеТекстаДокумента);
		КонецЕсли;
	Исключение
		Возврат;
	КонецПопытки;
	...
	Если ВидДокумента = "query" Тогда
		Если Действие = "execute-query" Тогда
			ВыполнитьКод("КодЗапроса", Неопределено, Алгоритм, Истина, ТекстДокумента);
		ИначеЕсли Действие = "execute-query-to-cursor" Тогда
			ВыполнитьКод("КодЗапроса", "ДоКурсора", Алгоритм, Истина, ТекстДокумента);
		ИначеЕсли Действие = "execute-query-package" Тогда
			ВыполнитьКод("КодЗапроса", "ПакетныйЗапрос", Алгоритм, Истина, ТекстДокумента);
		ИначеЕсли Действие = "get-query-code" Тогда
			ПолучитьКодДляВставки(Неопределено);
		КонецЕсли;
	ИначеЕсли ВидДокумента = "before-query" Тогда
		Если Действие = "execute-query" Тогда
			ВыполнитьКод("КодЗапроса", Неопределено, Алгоритм, Ложь, ТекстДокумента);
		КонецЕсли;
	ИначеЕсли ВидДокумента = "client" Тогда
