...
Функция ВыполнитьПолныйМодульНаКлиенте(Знач ПодготовленныйМодуль, Знач КонтекстВыполнения)
	...
	Попытка
		АдресИсполнителя = СформироватьИсполнительКодаКлиента(ПодготовленныйМодуль.Модуль);
		ИмяВременногоФайла = ПолучитьИмяВременногоФайла("epf");
		Если ПолучитьФайл(АдресИсполнителя, ИмяВременногоФайла, Ложь) = Ложь Тогда
			ВызватьИсключение "Не удалось получить временную обработку исполнителя кода.";
		КонецЕсли;

		СтадияОшибки = "Компиляция";
		ФормаИсполнитель = ВнешниеОбработки.ПолучитьФорму(ИмяВременногоФайла,
			"ИсполнительКодаКлиент", ЭтаФорма, Новый УникальныйИдентификатор);
	...
	Если ЗначениеЗаполнено(ИмяВременногоФайла) Тогда
		...
		КонецПопытки;
	КонецЕсли;
	ОсвободитьИсполнительКодаКлиента(АдресИсполнителя);

	Возврат Результат;
КонецФункции

&НаСервере
Функция СформироватьИсполнительКодаКлиента(Знач ТекстМодуля)
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("epf");
	Попытка
		...
		АдресИсполнителя = ПоместитьВоВременноеХранилище(ДвоичныеДанныеИсполнителя, Новый УникальныйИдентификатор);
	Исключение
		Если ЗначениеЗаполнено(ИмяВременногоФайла) Тогда
			Попытка
				УдалитьФайлы(ИмяВременногоФайла);
	...
	Исключение
	КонецПопытки;
	Возврат АдресИсполнителя;
КонецФункции

&НаСервере
Процедура ОсвободитьИсполнительКодаКлиента(Знач АдресИсполнителя)
	Если НЕ ЗначениеЗаполнено(АдресИсполнителя) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьВременнуюОбработку(ТекстМодуля, ИмяФайла, ТипМодуля = "Объект")
	...
	ТекстДляСлота = Символы.ВК + Символы.ПС + ТекстМодуля + Символы.ВК + Символы.ПС;
