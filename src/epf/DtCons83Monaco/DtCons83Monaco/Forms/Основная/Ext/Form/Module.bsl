...
Процедура ОбработатьАктивациюДокументаРабочейОбласти(ПараметрыСобытия)
	...
	Элементы.Алгоритмы.ТекущаяСтрока = Алгоритм.ПолучитьИдентификатор();
	УстановитьСтраницуКода(ИмяПоля);
	ТекущееПоле = ИмяПоля + Элементы.Алгоритмы.ТекущаяСтрока;
	УстановитьОтборЗначений();
	УстановитьЗаголовокФормы();
...
Процедура ОбработатьКомандуТаблицРабочейОбласти(ПараметрыСобытия)
	...
	Попытка
		АктивныйДокумент = QueryView().DataConsoleApp.getActiveDocument();
		Если АктивныйДокумент <> Неопределено Тогда
			ОбработатьАктивациюДокументаРабочейОбласти(АктивныйДокумент);
	...
	ОтобразитьРезультатВременнойТаблицы(СтрокаТаблиц.РезультатЗапроса, Алгоритм.КодЗапроса);
	ПозиционироватьВТекстеИмяТаблицы(СтрокаТаблиц.ИмяТаблицы);
...
Процедура ОбработатьКомандуРабочейОбласти(ПараметрыСобытия)
	...
	Если Не ЭтоДействиеВыполненияРабочейОбласти(Действие) Тогда
		ОбработатьИзменениеДокументаРабочейОбласти(ПараметрыСобытия);
	КонецЕсли;
	...
	Если ЭтоДействиеВыполненияРабочейОбласти(Действие) Тогда
		...
		Элементы.Значения.ОтборСтрок = Новый ФиксированнаяСтруктура("АлгоритмУИД", ТекущийАлгоритм);
	Иначе
		ОбработатьАктивациюДокументаРабочейОбласти(ПараметрыСобытия);
	КонецЕсли;
	...
	Если ВидДокумента = "query" Тогда
		...
		ИначеЕсли Действие = "get-query-code" Тогда
			ПолучитьКодДляВставки(Неопределено);
		КонецЕсли;
	ИначеЕсли ВидДокумента = "before-query" Тогда
		Если Действие = "execute-query" Тогда
	...
	ИначеЕсли ВидДокумента = "client" Тогда
		...
		ИначеЕсли Действие = "insert-iteration-example" Тогда
			...
			ОбновитьТекстДокументаHTML(АлгоритмУИД, ВидДокумента);
			ЭтотОбъект.Модифицированность = Истина;
...
Процедура ОбновитьРабочуюОбластьHTML()
	...
	Попытка
		...
		АктивныйДокумент = РабочаяОбласть.getActiveDocument();
		Если АктивныйДокумент <> Неопределено Тогда
			ОбработатьАктивациюДокументаРабочейОбласти(АктивныйДокумент);
		КонецЕсли;
	Исключение
