...
Процедура ЗаписатьСнимокРабочейОбластиВФайл(ПолноеИмяФайла, Знач Снимок)
	...
	ЗаписатьАлгоритмыСнимкаВФайлРекурсивно(Поток, ДанныеСнимка.algorithms, "");
...
Процедура ОбработатьИзменениеДокументаРабочейОбласти(ПараметрыСобытия)
	...
	Если ПустаяСтрока(АлгоритмУИД) Или ПустаяСтрока(ВидДокумента) Или ПустаяСтрока(ИдентификаторДокумента) Тогда
		Если Элементы.Алгоритмы.ТекущиеДанные <> Неопределено Тогда
			Элементы.Алгоритмы.ТекущиеДанные[СтраницаКода] = ПолучитьТекст(Ложь);
			АлгоритмыОбновитьФлагиСтроки(Элементы.Алгоритмы.ТекущиеДанные);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьРедактированиеПараметраРабочейОбласти(ПараметрыСобытия)
	...
	Элементы.Алгоритмы.ТекущаяСтрока = Алгоритм.ПолучитьИдентификатор();
	УстановитьОтборЗначений();
	Элементы.Значения.ТекущаяСтрока = ЗначениеПараметра.ПолучитьИдентификатор();
...
Процедура ОбработатьСохранениеРабочейОбласти(ПараметрыСобытия, СохранитьКак = Ложь)
	...
	Если ПараметрыСобытия.Свойство("workspaceJson", ЗначениеСнимка)
		И ТипЗнч(ЗначениеСнимка) = Тип("Строка")
