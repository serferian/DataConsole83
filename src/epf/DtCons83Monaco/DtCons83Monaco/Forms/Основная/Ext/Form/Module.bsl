...
Процедура ВыполнитьКод(ВыполнитьКодСоСтраницы, ДополнительныеПараметры = Неопределено,
	...
	Если СтраницаКода = "КодСервера1С" Тогда
		...
		Текст = ?(АлгоритмВыполнения = Неопределено,
			ПолучитьТекст(Истина), Алгоритм[СтраницаКода] + ПолучитьКодПросмотраПеременных());
		Результат = ВыполнитьКодНаСервере(Текст, Параметры);
		ОкончаниеВыполнения = ТекущаяУниверсальнаяДатаВМиллисекундах();
...
&НаСервере
Функция ВыполнитьКодНаСервере(Знач Текст, Знач Параметры = Неопределено)
	ПодготовленныйМодуль = ПодготовитьМодульКода(Текст);
...
&НаСервереБезКонтекста
Функция ПодготовитьМодульКода(Знач Текст)
