...
Функция ВыполнитьЗапросСЗамерами(КодЗапроса, ПараметрыЗапроса)
	...
	Для Каждого ПакетЗапроса Из СхемаЗапроса.ПакетЗапросов Цикл
		...
		Замеры.Добавить((ОкончаниеВыполненияПодзапроса - НачалоВыполненияПодзапроса) / 1000);
	КонецЦикла;
...
Процедура ВыгрузитьДанныеНаСервере(КодЗапроса, ПараметрыЗапроса = Неопределено)
	...
	Замеры = ВыполнитьЗапросСЗамерами(КодЗапроса, ПараметрыЗапроса);
	
	// переопределим НачалоВыполнения для реального замера выполнения запроса
	НачалоВыполнения = ТекущаяУниверсальнаяДатаВМиллисекундах();
...
Процедура УстановитьУсловноеОформлениеЗамеров()
	...
	ОбщаяДлительность = (ОкончаниеВыполнения - НачалоВыполнения) / 1000;
	ПределКрасный = ОбщаяДлительность * 0.4; // 40% - красный
	ПределЖелтый = ОбщаяДлительность * 0.1;
