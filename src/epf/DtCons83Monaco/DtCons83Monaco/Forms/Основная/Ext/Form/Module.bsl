...
Процедура ФорматироватьЗапросАлгоритма(Алгоритм, Блок = Неопределено)
	...
	Алгоритм.КодЗапроса = СокрЛП(Блок);// повторно уберем начальные и конечные пробелы;
	АлгоритмыОбновитьФлагиСтроки(Алгоритм);
...
&НаКлиенте
Процедура ОткрытьКонструкторЗапросаДляАлгоритма(Алгоритм)
	...
	Попытка
		...
		Оповещение = Новый ОписаниеОповещения("УстановитьТекстЗапроса", ЭтотОбъект, Алгоритм);
...
&НаКлиенте
Процедура ОпределитьПараметрыЗапросаДляАлгоритма(Алгоритм)
...
Процедура ОбработатьИзменениеДокументаРабочейОбласти(ПараметрыСобытия)
	...
	Попытка
		Алгоритм[ИмяПоля] = QueryView().DataConsoleApp.getDocumentText(ИдентификаторДокумента);
		АлгоритмыОбновитьФлагиСтроки(Алгоритм);
	Исключение
		ВывестиОшибку("Не удалось сохранить изменения документа HTML-редактора: " + ОписаниеОшибки(), Ложь);
	КонецПопытки;
...
Процедура ОбработатьКомандуРабочейОбласти(ПараметрыСобытия)
	...
	Если ВидДокумента = "query" И Действие = "format-query" Тогда
		ФорматироватьЗапросАлгоритма(Алгоритм);
...
Процедура ОбновитьТекстДокументаHTML(АлгоритмУИД, ВидДокумента)
	...
	Операция.Вставить("text", Алгоритм[ИмяПоля]);
	Операция.Вставить("hasContent", ЗначениеЗаполнено(Алгоритм[ИмяПоля]));
