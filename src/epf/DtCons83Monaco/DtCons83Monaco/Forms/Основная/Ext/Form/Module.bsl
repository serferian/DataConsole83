...
Процедура ЗаписатьСнимокРабочейОбластиВФайл(ПолноеИмяФайла, Знач Снимок)
	ТекстСнимка = СокрЛП(Снимок);
	Если ПустаяСтрока(ТекстСнимка)
		Или Лев(ТекстСнимка, 1) <> "{"
		Или Прав(ТекстСнимка, 1) <> "}" Тогда
		ВызватьИсключение "Снимок рабочей области имеет неверный формат.";
	КонецЕсли;
	ЗаписьJSON = Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку();
	ЗаписатьJSON(ЗаписьJSON, СформироватьЗначенияПараметровРабочейОбласти());
	JSONПараметров = ЗаписьJSON.Закрыть();
	ТекстСнимка = Лев(ТекстСнимка, СтрДлина(ТекстСнимка) - 1)
		+ "," + Символ(34) + "parameterValues" + Символ(34) + ":" + JSONПараметров + "}";
...
Функция СформироватьЗначенияПараметровРабочейОбласти()
	ЗначенияДляСериализации = Новый Массив;
	Для Каждого ЗначениеПараметра Из ЭтотОбъект.Значения Цикл
		ЗначениеДляСериализации = Новый Структура();
		ЗначениеДляСериализации.Вставить("algorithmId", Строка(ЗначениеПараметра.АлгоритмУИД));
		ЗначениеДляСериализации.Вставить("name", ЗначениеПараметра.Имя);
		ЗначениеДляСериализации.Вставить("value", ЗначениеПараметра.Значение);
		ЗначенияДляСериализации.Добавить(ЗначениеДляСериализации);
	КонецЦикла;
	Возврат СериализоватьЗначенияПараметровНаСервере(ЗначенияДляСериализации);
КонецФункции

&НаСервереБезКонтекста
Функция СериализоватьЗначенияПараметровНаСервере(ЗначенияДляСериализации)
	Результат = Новый Массив;
	Для Каждого ЗначениеДляСериализации Из ЗначенияДляСериализации Цикл
		Описание = Новый Структура();
		Описание.Вставить("algorithmId", ЗначениеДляСериализации.algorithmId);
		Описание.Вставить("name", ЗначениеДляСериализации.name);
		Описание.Вставить("value", СериализоватьЗначениеПараметраНаСервере(
			ЗначениеДляСериализации.value));
