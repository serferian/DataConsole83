...
Процедура ПозиционироватьВТекстеИмяТаблицы(ИмяТаблицы)
	...
	Если СтрНачинаетсяС(ИмяТаблицы, "РЕЗУЛЬТАТ") Тогда
		...
		КонецЦикла;
		НачальнаяПозиция = ПозицияРазделителя;
		НомерСтрокиПозиции = СтрЧислоВхождений(Лев(ТекстЗапроса, ПозицияРазделителя), Символы.ПС) + 1;
		View().revealLineInCenter(НомерСтрокиПозиции);
		View().setSelectionByLength(НачальнаяПозиция, НачальнаяПозиция);
		Возврат;
	Иначе
