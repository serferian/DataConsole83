...
Процедура ОбработатьКомандуРабочейОбласти(ПараметрыСобытия)
	...
	Если ВидДокумента = "query" Тогда
		...
		ИначеЕсли Действие = "fill-parameters" Тогда
			ОпределитьПараметрыЗапроса(Неопределено);
			ОбновитьПараметрыАлгоритмаHTML(АлгоритмУИД);
		ИначеЕсли Действие = "format-query" Тогда
			КодФорматироватьЗапрос(Неопределено);
