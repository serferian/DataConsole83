...
Процедура ВыполнитьКод(ВыполнитьКодСоСтраницы, ДополнительныеПараметры = Неопределено,
	...
	Если СтраницаКода = "КодКлиента1С" Тогда
		...
		Текст = ?(АлгоритмВыполнения = Неопределено,
			ПолучитьТекст(Истина), Алгоритм[СтраницаКода] + ПолучитьКодПросмотраПеременных());
		Результат = ВыполнитьКодНаКлиенте(Текст, Параметры);
	...
	Если СтраницаКода = "КодСервера1С" Тогда
		...
		Текст = Текст + КодПросмотраПеременных;
...
&НаКлиенте
Функция ВыполнитьКодНаКлиенте(Знач Текст, Знач Параметры = Неопределено)
	Возврат ВыполнитьКодСтраницыНаКлиенте(Текст, Параметры);
КонецФункции
...
Функция ВыполнитьКодНаСервере(Знач Текст, Знач Параметры = Неопределено, Знач КодПросмотраПеременных = "")
	...
	Если НЕ ПодготовленныйМодуль.ЕстьМетоды Тогда
		Возврат ВыполнитьКодСтраницы(Текст, Параметры);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПодготовитьМодульКода(Знач Текст, Знач КодПросмотраПеременных = "")
	...
	Для Индекс = 0 По СтрокиКода.ВГраница() Цикл
		...
		КонецЕсли;
		Если ЭтоДирективаПрепроцессора И НЕ ПризнакиСтрок[Индекс] Тогда
			ОсновнойКод.Добавить(Новый Структура("Текст, НомерСтроки", СтрокаКода, Индекс + 1));
		КонецЕсли;
	...
	Возврат Новый Структура("ЕстьМетоды, Модуль, КартаСтрок, КоличествоСтрокИсходногоКода",
		ЕстьМетоды, СтрСоединить(СгенерированныеСтроки, Символы.ВК + Символы.ПС), КартаСтрок, СтрокиКода.Количество());
КонецФункции
...
&НаСервере
Процедура ЗаписатьВременнуюОбработку(ТекстМодуля, ИмяФайла)
	Макет = ПолучитьМакет("ИсполнительКодаСервер");
	ШестнадцатеричныеДанные = ПолучитьHexСтрокуИзДвоичныхДанных(Макет);

	МаркерНачала = ПолучитьHexСтрокуИзДвоичныхДанных(ПолучитьДвоичныеДанныеИзСтроки("// __DC83_MODULE_SLOT_START__", КодировкаТекста.UTF8, Ложь));
	МаркерКонца = ПолучитьHexСтрокуИзДвоичныхДанных(ПолучитьДвоичныеДанныеИзСтроки("// __DC83_MODULE_SLOT_END__", КодировкаТекста.UTF8, Ложь));
