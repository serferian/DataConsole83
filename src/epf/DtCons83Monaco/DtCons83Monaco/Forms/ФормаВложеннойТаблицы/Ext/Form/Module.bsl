...
#Область ОбработчикиСобытийЭлементовТаблицыФормыВложеннаяТаблица
...
Процедура ВложеннаяТаблицаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если НЕ РежимРедактирования Тогда
		...
		Если СодержимоеЯчейки = "ТаблицаЗначений" И ЗначениеЗаполнено(Адрес) Тогда
			ПараметрыОткрытия = Новый Структура("Заголовок, АдресТаблицы", Заголовок + ": " + имяТаблицы, Адрес);
			ОткрытьФорму("ВнешняяОбработка.DtCons83Monaco.Форма.ФормаВложеннойТаблицы", ПараметрыОткрытия, ЭтотОбъект, Истина);
		Иначе
